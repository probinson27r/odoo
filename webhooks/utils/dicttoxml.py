...
def convert(obj, ids, attr_type, parent='data'):
    ...
    if isinstance(obj, numbers.Number) or type(obj) in (str, unicode):
        return convert_kv('record', obj, attr_type)
    if hasattr(obj, 'isoformat'):
        return convert_kv('record', obj.isoformat(), attr_type)
    if type(obj) == bool:
        return convert_bool('record', obj, attr_type)
    if obj == None:
        return convert_none('record', '', attr_type)
...
def convert_list(items, ids, parent, attr_type):
    ...
    for i, item in enumerate(items):
        LOG.info('Looping inside convert_list(): record="%s", type="%s"' % (unicode_me(item), type(item).__name__))
        attr = {} if not ids else { 'id': '%s_%s' % (this_id, i+1) }
        if isinstance(item, numbers.Number) or type(item) in (str, unicode):
            addline(convert_kv('record', item, attr_type, attr))
        elif hasattr(item, 'isoformat'): # datetime
            addline(convert_kv('record', item.isoformat(), attr_type, attr))
        elif type(item) == bool:
            addline(convert_bool('record', item, attr_type, attr))
        elif isinstance(item, dict):
            if not attr_type:
                addline('<record>%s</record>' % (convert_dict(item, ids, parent, attr_type)))
            else:
                addline('<record type="dict">%s</record>' % (convert_dict(item, ids, parent, attr_type)))
        elif isinstance(item, collections.Iterable):
            if not attr_type:
                addline('<record %s>%s</record>' % (make_attrstring(attr), convert_list(item, ids, 'record', attr_type)))
            else:
                addline('<record type="list"%s>%s</record>' % (make_attrstring(attr), convert_list(item, ids, 'record', attr_type)))
        elif item is None:
            addline(convert_none('record', None, attr_type, attr))
        else:
...
def dicttoxml(obj, root=True, custom_root='data', ids=False, attr_type=True):
